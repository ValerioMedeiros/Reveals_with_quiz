  ...
  init(function() {
    $(function () {
      ...
      for (var i = 0, len = quizzes.length; i < len; i++) {
        ...
        var currQuiz;
        const quizName = quizNode.getAttribute("data-quiz");
        if (quizName.length > 0) currQuiz = window[quizName];
        else currQuiz = quiz;
        options.json = currQuiz;
...
function createQuizStructure(quizNode, classname) {
  ...
  quizDiv.innerHTML = `
  <div id="` + classname + `">
          <h2 class="quizName"><!-- where the quiz name goes --></h2>
          <div class="quizArea">
